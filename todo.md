...
- [x] **OPPORTUNITY-INGESTION-01 / staging read-only** — service BOAMP derrière `PublicNoticeSearchPort` et script `scripts/ingest_boamp_opportunities.py` publiés ; pagination, budget, filtrage, expiration et déduplication sont bornés. La persistence des observations, le scoring et la qualification restent les incréments suivants.
