...
# Compatibility exports remain available while callers migrate to the module-owned models.
...
class PricingImportBatchRecord(TenantScopedRecord, Base):
    ...
    __table_args__ = (
        ...
        sa.CheckConstraint("valid_row_count >= 0 AND valid_row_count <= row_count", name="valid_rows_bound"),
...
class PricingImportRowRecord(TenantScopedRecord, Base):
    ...
    __table_args__ = (
        sa.ForeignKeyConstraint(
            ["tenant_id"], ["tenants.id"], name="fk_pricing_import_rows__tenant", ondelete="RESTRICT"
        ),
        sa.ForeignKeyConstraint(
            ["tenant_id", "batch_id"],
