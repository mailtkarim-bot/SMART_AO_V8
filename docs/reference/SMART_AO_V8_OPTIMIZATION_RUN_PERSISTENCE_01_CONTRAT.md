...
Les tests unitaires et les contrôles statiques sont exécutables dans le sandbox. La migration PostgreSQL et le test d’intégration transactionnel exigent un serveur PostgreSQL; aucun serveur n’est disponible dans le sandbox courant, donc aucune réussite PostgreSQL réelle ne doit être déclarée avant la recette sur Docker ou machine cible.
