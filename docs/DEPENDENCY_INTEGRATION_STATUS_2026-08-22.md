...
| Profil de veille opportunité | Bounded context `opportunity` commencé ; `WatchProfileCriteria` pur, commande de création, snapshot/hash canonique et capabilities patronales `opportunity.profile.read/write` | **Fondation domaine/application codée** ; persistence versionnée, lecture HTTP, ingestion BOAMP, scoring explicable et conversion contrôlée en Case restent les lots S10 suivants. |
