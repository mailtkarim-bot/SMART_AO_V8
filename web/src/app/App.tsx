      ...
      <main className="content">
        ...
        <section className="section-block draft-section" id="draft-section"><div className="section-heading"><div><span className="section-kicker">CHIFFRAGE PRIVÉ</span><h2>Brouillon financier</h2></div>{draft && <span className="draft-status"><span className="status-dot" />DRAFT · Révision {draft.aggregate_revision}</span>}</div><div className="draft-toolbar"><label><span>Affaire sélectionnée</span><select value={selectedCaseId} onChange={(event) => setSelectedCaseId(event.target.value)}><option value="">Choisir une affaire</option>{cases.map((item) => <option key={item.case_id} value={item.case_id}>{item.work_label}</option>)}</select></label><label className="report-input"><span>Identifiant du brouillon</span><input value={reportId} onChange={(event) => setReportId(event.target.value)} placeholder="UUID du snapshot DRAFT" /></label><div className="draft-actions"><button className="secondary-button" onClick={() => void createDraft()} disabled={loadingDraft}>+ Nouveau brouillon</button><button className="primary-button load-button" onClick={() => void loadDraft()} disabled={loadingDraft}>{loadingDraft ? "Chargement…" : "Lire le brouillon"}<span>→</span></button></div></div>
          <div className="detail-panel import-commit-panel">
            <div className="panel-heading"><div><h3>Importer dans le brouillon</h3><p>Seules les lignes validées du batch DPGF/BPU/Excel sont appliquées, côté patron.</p></div><span className={`state-badge state-${pricingImportState.toLowerCase()}`}>{pricingImportState === "IDLE" ? "PRÊT" : pricingImportState}</span></div>
            <div className="import-commit-grid">
              <label><span>Batch d’import validé</span><input value={pricingImportBatchId} onChange={(event) => setPricingImportBatchId(event.target.value)} placeholder="UUID du batch PREVIEWED" /></label>
              <label><span>Révision du batch</span><input type="number" min="1" step="1" value={pricingImportBatchRevision} onChange={(event) => setPricingImportBatchRevision(event.target.value)} /></label>
              <label><span>Révision du brouillon</span><input type="number" min="0" step="1" value={pricingImportReportRevision} onChange={(event) => setPricingImportReportRevision(event.target.value)} /></label>
              <button className="primary-button" type="button" onClick={() => void commitPricingImport()} disabled={!selectedCaseId || !reportId.trim() || !pricingImportBatchId.trim()}>Commiter les lignes validées <span>→</span></button>
            </div>
            <small className="invariant-note">Le serveur verrouille le batch et le brouillon, refuse les erreurs, applique l’idempotence et ne retourne aucun montant dans ce receipt.</small>
          </div>
