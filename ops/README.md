...
| `ops/run-knowledge-embeddings-preprod.sh` | Indexation BGE one-shot par tenant/Case/version DCE, avec env protégé et opt-in RAG séparé. |
| `ops/run-dce-analysis-preprod.sh` | Analyse RC one-shot d’une version DCE admise et vérifiée, avec acteur SYSTEM, profil Compose isolé et sortie limitée au reçu technique. |
...
L’indexation RAG opérateur est préparée par `ops/run-knowledge-embeddings-preprod.sh TENANT_ID CASE_ID DCE_VERSION_ID`. Elle reste one-shot et ne démarre aucun service permanent. Le fichier d’environnement doit conserver `SMART_AO_RAG_ENABLED=0` et `SMART_AO_RAG_INDEXING_ENABLED=0` par défaut ; l’opérateur ne passe à `1` qu’après préchargement local du modèle et validation du corpus. Le modèle est chargé sans téléchargement implicite lorsque `SMART_AO_BGE_LOCAL_FILES_ONLY=1`. Le worker ne restitue aucun fragment, extrait, embedding ou montant dans sa sortie.

L’analyse déterministe du règlement de consultation est exécutée séparément par `ops/run-dce-analysis-preprod.sh TENANT_ID DCE_VERSION_ID`. Le service `dce-rc-analysis-runner` appartient au profil Compose `dce-analysis` et n’est jamais lancé par `up -d` du stack standard. Le service relit exclusivement les fragments d’extraction `COMPLETED` de la version DCE admise et vérifiée ; l’acteur est fixé à `SYSTEM` dans le service applicatif, et le wrapper refuse les identifiants qui ne sont pas des UUID ou un fichier d’environnement qui n’est pas en mode `0600`. La sortie ne contient que le statut, le code résultat, les identifiants techniques et le nombre d’événements : aucun extrait, nom de fichier, hash d’original, prix ou contenu DCE ne doit être imprimé.

Lorsque `SMART_AO_SMTP_ENABLED=1`, le service `submission-export-smtp-worker` doit aussi recevoir `SMART_AO_INSTALL_NOTIFICATIONS=1`, `SMART_AO_SMTP_TO` et les identifiants SMTP uniquement depuis le fichier d’environnement protégé ou le gestionnaire de secrets. Ce destinataire est un opérateur unique configuré hors du dossier client ; le worker ne lit jamais une adresse depuis la requête HTTP et n’envoie ni document, ni archive, ni montant. Sans SMTP activé, il marque les messages comme `skipped` afin d’éviter une accumulation volontaire. La présence d’un message publié ne constitue pas une preuve de remise : un serveur de test dédié, TLS valide et une vérification de délivrabilité restent requis sur l’environnement réel.
...
Le template fournit désormais le script de déploiement, les scripts backup/restore/healthcheck/rotation, les timers systemd, la rotation des logs, les healthchecks live/ready détaillés, le pinning par digest, le worker outbox SMTP optionnel et l’antenne RAG BGE one-shot. Restent requis avant un premier client : exécution sur un VPS réel, transfert des backups hors VPS, supervision externe, firewall administré, test EICAR ClamAV, test SMTP vers une boîte dédiée, rotation de secrets avec fenêtre validée et rapport de restauration. Le sandbox ne possède pas Docker et aucun VPS réel, serveur SMTP, accusé de remise ou stack complet n’a encore exécuté le déploiement.
