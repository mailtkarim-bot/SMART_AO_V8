...
if [[ $# -ne 3 ]]; then
  echo "usage: $0 TENANT_ID DCE_VERSION_ID DCE_RC_ANALYSIS_ID" >&2
  exit 64
fi

tenant_id="$1"
dce_version_id="$2"
dce_rc_analysis_id="$3"

for value_name in tenant_id dce_version_id dce_rc_analysis_id; do
  value="${!value_name}"
  if [[ ! "$value" =~ ^[0-9a-fA-F-]{36}$ ]]; then
    echo "invalid ${value_name//_/-} UUID" >&2
    exit 64
  fi
done
...
exec docker compose --env-file "$ENV_FILE" -p "$PROJECT_NAME" -f "$COMPOSE_FILE" \
  ...
  python -m app.workers.dce_requirements \
  --tenant-id "$tenant_id" \
  --dce-version-id "$dce_version_id" \
  --dce-rc-analysis-id "$dce_rc_analysis_id"
